...
/******************************
* masonry projects block
******************************/

function lolfmk_print_masonry_projects_admin($item = null) {
	...
	$project_number = (lolfmk_find_xml_value($item, 'project-number')) ? lolfmk_find_xml_value($item, 'project-number') : '8';
	$project_category = (lolfmk_find_xml_value($item, 'project-category')) ? lolfmk_find_xml_value($item, 'project-category') : '';
	$portfolio_layout = (lolfmk_find_xml_value($item, 'portfolio-layout')) ? lolfmk_find_xml_value($item, 'portfolio-layout') : '';
	$portfolio_columns = (lolfmk_find_xml_value($item, 'portfolio-columns')) ? lolfmk_find_xml_value($item, 'portfolio-columns') : '';
	...
	echo '<div class="page-item item-masonry-projects item-1-1" data-type="item-masonry-projects" data-item="Masonry-Projects">';

	lolfmk_item_btns(__('Masonry-Projects', 'lollum'), 'yes');
	lolfmk_item_before_inner(__('Masonry-Projects', 'lollum'));
	...
	$ob_portfolio_layout = array(
		'name' => __('Portfolio Layout', 'lollum'),
		'data' => 'portfolio-layout',
		'value' => $portfolio_layout,
		'options' => array('grid', 'full'),
		'desc' => __('Select the layout of the block', 'lollum')
	);

	lolfmk_pb_simple_select($ob_portfolio_layout);

	/*** end command ***/

	/*** begin command ***/

	$ob_portfolio_columns = array(
	...
	lolfmk_pb_simple_select($ob_portfolio_columns);
